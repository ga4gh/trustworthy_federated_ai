...
Make sure MinIO is accessible from the host,
Then run:
...
The main entry point for understanding the current federated training workflow is **`run_orch.py`**. Start there to see how training rounds are created, how TES tasks are dispatched to the central and client sites, and how the resulting model updates are aggregated.
...
│       ├── 00-cluster.yaml
│       ├── 01-rbac.yaml
│       ├── 02-funnel-config.yaml
│       ├── 03-minio.yaml
│       ├── 04-site-central.yaml
│       ├── 05-client-sites.yaml
│       └── 06-seeder-job.yaml
│
├── scripts/
│   ├── get_flan_data.py
│   ├── prepare_splits_after_flan.py
│   └── old_data_scripts/
│
├── src/
│   ├── client.py
...
This repository was developed from scratch as part of **Vidit Khandelwal (@TheVidz)**'s **Google Summer of Code 2026** project with **GA4GH**, titled **“GA4GH-aligned Trustworthy Federated AI”**

Under the mentorship of **Andrew Gearhart**, Vidit implemented the federated learning pipeline and its supporting infrastructure, including:

* Federated ancestry classification and model training.
* DRS-based genomic data resolution and TES/Funnel-based distributed execution.
* Federated orchestration, FedAvg aggregation, and model evaluation.
* MinIO-based checkpoint and metrics storage.
